...
def toDict(gumpIndex=-1):
    ...
    def _gumpList2Dict(strToList, dictKey, keys):
        ...
        tokenList = [int(token) if str.isdigit(token) else token for token in reTokens.split(strToList) if
                     token not in keys]
    ...
    for line in lines:
        ...
        if pieces[0]:
            if pieces[0] == 'Serial':
                ...
                _gumpList2Dict(pieces[1], 'XmfHTMLGumpColor',
                               ['X', 'Y', 'Width', 'Height', 'ClilocID', 'Background', 'scrollbar', 'Hue', 'ElemNum',
                                'ClilocText'])
            elif pieces[0].find('XmfHTMLTok') != -1:
                _gumpList2Dict(pieces[1], 'XmfHTMLTok',
                               ['X', 'Y', 'Width', 'Height', 'Background', 'scrollbar', 'Color', 'ClilocID',
                                'Arguments', 'ElemNum', 'ClilocText'])
                ...
                _gumpList2Dict(pieces[1], 'GumpButton',
                               ['X', 'Y', 'Released_ID', 'Pressed_ID', 'Quit', 'Page_ID', 'Return_value', 'Page',
                                'ElemNum'])
                ...
                _gumpList2Dict(pieces[1], 'CroppedText',
                               ['X', 'Y', 'Width', 'Height', 'Color', 'Text_ID', 'Page', 'ElemNum'])
            elif pieces[0].find('XmfHtmlGump') != -1:
                _gumpList2Dict(pieces[1], 'XmfHtmlGump',
                               ['X', 'Y', 'Width', 'Height', 'ClilocID', 'Background', 'scrollbar', 'Page', 'ElemNum',
                                'ClilocText'])
...
def clickButtonByCliclocPage(clilocID, rangeX, rangeY, page=1, gumpIndex=-1, gumpID=0, wait=True):
    ...
    if not isinstance(clilocID, int):
        raise TypeError(
            'clickButtonByClicloc is expecting an iteger as clilocID argument, it got an {0}'.format(type(int)))
